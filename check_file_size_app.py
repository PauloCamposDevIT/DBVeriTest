...
def process_server(srv):
    ...
    try:
        ...
        if not databases:
            ...
            # Para cada base, verifica as anomalias de tamanho de backup e agrupa os resultados
            for db in databases:
                backups = get_backups(cursor, db)
                if not backups:
                    continue
                anomalies = check_file_size(db, backups)
                if anomalies:
                    # Atualiza o campo "instance" utilizando o nome da instância obtido
        ...
        with PRINT_LOCK:
            print(f"\nErro ao ligar ao servidor {server}: {e}")
        connection_anomaly = {
            'database': 'N/A',
...
def main():
    ...
    if overall_anomalias:
        with PRINT_LOCK:
            print(f"\nTotal de anomalias detetadas: {len(overall_anomalias)}")
        log_anomalies(overall_anomalias)
    else:
        with PRINT_LOCK:
            print("\nNão foram encontradas nenhuma anomalia de tamanho de backups!")
